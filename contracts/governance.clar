...
(define-constant ERR-INVALID-MAJORITY-THRESHOLD u103)
...
(define-constant ERR-INVALID-TARGET-CONTRACT u114)
(define-constant ERR-INVALID-PARAM u115)
(define-constant ERR-MAX-PROPOSALS-EXCEEDED u116)
(define-constant ERR-INVALID-START-DELAY u117)
(define-constant ERR-INVALID-VOTE-WEIGHT u118)
(define-constant ERR-NOT-TOKEN-HOLDER u119)
(define-constant ERR-INVALID-DESCRIPTION-LENGTH u120)
...
(define-private (validate-majority-threshold (threshold uint))
  (if (and (> threshold u50) (<= threshold u100))
    (ok true)
    (err ERR-INVALID-MAJORITY-THRESHOLD))
)
...
(define-private (calculate-quadratic-weight (tokens uint))
  (let ((sqrt-tokens (unwrap-panic (pow tokens u2))))
    (/ sqrt-tokens u100))
)
...
(define-public (vote-on-proposal (id uint) (vote-for bool))
  (let (
    ...
    (asserts! (>= block-height (get start-block proposal)) (err ERR-PROPOSAL-NOT-ACTIVE))
    ...
    (asserts! (> balance u0) (err ERR-NOT-TOKEN-HOLDER))
...
(define-public (execute-proposal (id uint))
  (let (
    ...
    (asserts! (>= total-votes quorum) (err ERR-INVALID-QUORUM-THRESHOLD))
    (asserts! (>= majority (var-get majority-threshold)) (err ERR-INVALID-MAJORITY-THRESHOLD))
    (if (>= block-height timelock-end)
      (let (
        (target (unwrap! (get target-contract proposal) (err ERR-INVALID-TARGET-CONTRACT)))
        (fname (unwrap! (get function-name proposal) (err ERR-INVALID-PARAM)))
        ...
        (match (as-contract (contract-call? target fname param))
          success (begin
            ...
            (ok true))
          failure (err ERR-EXECUTION-FAILED))
      )
      (begin
